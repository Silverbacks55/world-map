...
function renderDomesticTab() {
    ...
    return `<div class="results-section-label">Regulations in <strong>${esc(wizardProfile.hq)}</strong> relevant to your sector — ${regs.length} found</div>
...
function renderImportExportTab() {
    ...
    let html = '';
    if (sources.length > 0) {
        html += `<div class="results-section-label">📥 Sourcing Country Exposure — ${sources.length} regulations in countries you import from</div>`;
        html += sources.map(r => regCard(r)).join('');
    }
    if (exportRegs.length > 0) {
        html += `<div class="results-section-label" style="margin-top:24px;">📤 Export Country Exposure — ${exportRegs.length} regulations in countries you sell to</div>`;
...
function renderIncentivesTab() {
    ...
    return `<div class="results-section-label">Biomethane &amp; renewable gas incentive programs in your countries — ${programs.length} found</div>
        ${programs.map(p => {
...
function renderVCMTab() {
    ...
    return `<div class="results-section-label">VCM projects in your countries matching your pollutant profile — ${projects.length} found</div>
        ${projects.map(p => {
            const pJson = JSON.stringify(p).replace(/'/g, "\\'").replace(/"/g, '&quot;');
...
function renderPollutantExposureTab() {
    ...
    return `
        <div class="results-section-label">Estimated super pollutant emissions profile for <strong>${esc(sector)}${subsectorNote}</strong></div>
